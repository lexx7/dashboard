...
- **Decision**: Выборка `WHERE (updated_at, sku) > (:cursor, :lastSku) ORDER BY
  ...
  читаем только записи с `updated_at < now() - interval '30 seconds'`. Размер батча
  настраивается в `application.yml`, по умолчанию 1000 записей (plan.md §3).
...
- **Decision**: Spring `@Scheduled` для периодических прогонов с интервалом опроса
  источника 60 секунд (настраивается в `application.yml`, plan.md §3) + REST-эндпоинты
  ручного запуска (plan.md §5). Конкурентные запуски исключены: один активный прогон
